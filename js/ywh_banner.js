...
(function (global) {
  ...
  function showButton () {
    if (prebutton) {
      y(prebutton).style.opacity = 1;
    }
    if (nextbutton) {
      y(nextbutton).style.opacity = 1;
    }
  }

  function hideButton () {
    if (prebutton) {
      y(prebutton).style.opacity = 0;
    }
    if (nextbutton) {
      y(nextbutton).style.opacity = 0;
    }
  }
  ...
  global.banner = {
    ...
    showAuto: function () {
      var ywh_this = this;
      if (this.bannerTimer) {
        clearTimeout(ywh_this.bannerTimer);
      }
    ...
    init: function (obj, json) {
      ...
      if (json) {
        ...
        if (json.pagination) {
          for (var i = 1; i <= this.max; i++) {
            var pagination = `<span class=pagination_bullet id=bullet${i} path=${i}></span>`;
            $(json.pagination).append(pagination);
